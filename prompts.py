...
PTE_SYSTEM = (
    ...
    f"|entry - invalidation| / entry x 100. Jika < {_STOP}: pindahkan invalidation ke level "
    "STRUKTUR berikutnya (di bawah swing low/order block untuk long; di atas swing high untuk "
    f"short), hitung ulang R:R; jika struktur tidak memberi stop >= {_STOP}% dengan R:R >= {_RR} "
    "-> no_trade. Governor menolak stop mikro tanpa kompromi — stop di dalam noise membuat "
    "realized risk jauh melebihi rencana.\n\n"
    ...
    f"TP1 minimal R:R {_RR}:1 (posisi ditutup PENUH di TP1); targets[1] opsional referensi.\n\n"
